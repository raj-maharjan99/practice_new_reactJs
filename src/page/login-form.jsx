import { useUserLoginMutation } from "@/auth/authApi";
...
import { setUserToLocal } from "@/slices/userSlice";
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        toast.success(response.status, { toastId: response?.data?.id });
        dispatch(setUserToLocal(response?.data));
        nav("/");
        // console.log("Login successful:", response);
      } catch (error) {
        console.error("Login failed:", error);
        toast.error(error?.data?.message || error?.data?.errors[0].msg);
      }
    },
  });
      ...
      <div className="w-full max-w-md p-8 space-y-6 bg-white shadow-lg rounded-xl">
        ...
        <form className="space-y-4" onSubmit={formik.handleSubmit}>
          ...
          <div className="flex items-center justify-between">
            ...
            <a
              href="#"
              className="text-sm text-blue-600 hover:underline focus:ring-2 focus:ring-blue-500"
            >
              Forgot Password?
            </a>
          </div>
